...
data['url'] = 'https://play.google.com/store/apps/details?id=idle.city.builder'
data['url_ios'] = 'https://apps.apple.com/de/app/idle-city-builder-tycoon/id1562656077'
template_name = 'app-store-main.tmpl'
...
str_template = re.sub("{{meta-description}}", data['meta_description'], str_template)
...
str_template = re.sub("{{url}}", data['url_ios'], str_template)
...
str_template = re.sub("{{div_4}}", gp.get_divs(param='<a href="'+data['url_ios']+'" class="knopa"><img src="images/app-store.png" height="60%" width="60%"></a>'), str_template)
str_out = re.sub("{{orgn}}", rnd_str_13, str_template)

f = open(path+'white_ios.php', 'w+', encoding='utf8')
f.write(str_out)
f.close()
